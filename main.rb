...
post '/new-with-key', :provides => [:html, :json] do
  lock_id = UUIDTools::UUID.random_create.to_s
...
# Lock the lock
post '/lock/:id', :provides => [:html, :json] do |lock_id|
...
def open_db
  unless File.exist?(DB_FILE)
    abort("Database file `#{DB_FILE}` does not exist. Maybe run `ruby create_database.rb`")
  end
